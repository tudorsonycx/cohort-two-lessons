...
class Butterfly(val name: String, val age: Int, val dietType: DietType, val numberOfLegs: Int, val wingSpan: Int)
...
object Butterfly {
  def apply(name: String, age: Int, dietType: DietType, numberOfLegs: Int, wingSpan: Int) =
    new Butterfly(name, age, dietType, numberOfLegs, wingSpan)
}